...
/**
 * This is a list of all native mono functions.
 * Some of them are needed, some of them may not be used at all.
 * If you need one of them for some reason, head to: http://docs.go-mono.com/,
 * and search for the function to know its signature.
 * Or look into the Mono sourcecode in github. Or just google the function.
 */
type NativeSignatures = {
  [name: string]: [NativeType, NativeType[]?, (NativeFunctionOptions | NativeABI)?] | null
}

const NATIVE_SIGNATURES: NativeSignatures = {
  g_free: null,
...
for (const key of keys) {
  ...
  natives[key] = new ExNativeFunction(address, NATIVE_SIGNATURES[key][0], NATIVE_SIGNATURES[key][1], NATIVE_SIGNATURES[key][2])
}
